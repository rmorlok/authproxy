...
func (s *service) GetCurrentEncryptionKeyVersionForScope(ctx context.Context, scope string) (*EncryptionKeyVersion, error) {
	var result EncryptionKeyVersion
	...
	if err != nil {
		...
		return nil, errors.Wrap(err, "failed to get current encryption key version for scope")
	}

	return &result, nil
}

func (s *service) ListEncryptionKeyVersionsForScope(ctx context.Context, scope string) ([]*EncryptionKeyVersion, error) {
	var result EncryptionKeyVersion
...
func (s *service) GetMaxOrderedVersionForScope(ctx context.Context, scope string) (int64, error) {
...
func (s *service) ClearCurrentFlagForScope(ctx context.Context, scope string) error {
	now := apctx.GetClock(ctx).Now()
	...
	if err != nil {
		return errors.Wrap(err, fmt.Sprintf("failed to clear current flag for scope %s", scope))
	}
...
func (s *service) EnumerateEncryptionKeyVersions(
	ctx context.Context,
