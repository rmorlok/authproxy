...
func (c *connectorResource) fields() []any {
	return []any{
		&c.Id,
		&c.Namespace,
		&c.Name,
...
func (c *connectorResource) values() []any {
	return []any{
		c.Id,
		c.Namespace,
		c.Name,
...
func (c *connectorResource) validate() error {
	if c.Id.IsNil() {
		return errors.New("connector id is required")
	}
	if err := c.Id.ValidatePrefix(apid.PrefixConnectorVersion); err != nil {
...
func (s *service) ensureConnectorResourceForVersion(
	ctx context.Context,
	tx *sql.Tx,
	cv *ConnectorVersion,
) error {
	sqb := s.sq.RunWith(tx)
	var existing connectorResource
	err := sqb.
...
func (s *service) UpdateConnectorName(ctx context.Context, id apid.ID, name scommon.ResourceName) error {
	...
	if err := id.ValidatePrefix(apid.PrefixConnectorVersion); err != nil {
		return fmt.Errorf("invalid connector id: %w", err)
	}
