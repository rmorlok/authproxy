...
func TestSearchResourcesRanksNamesBeforeLabelMatches(t *testing.T) {
	_, db, raw := MustApplyBlankTestDbConfigRaw(t, nil)

	exactID := apid.New(apid.PrefixActor)
	prefixID := apid.New(apid.PrefixActor)
	labelID := apid.New(apid.PrefixActor)
	systemOnlyID := apid.New(apid.PrefixActor)
	deletedID := apid.New(apid.PrefixActor)

	inserts := []string{
		fmt.Sprintf(`INSERT INTO actors (id, name, namespace, labels, external_id, created_at, updated_at) VALUES ('%s', 'acme', 'root', '{"literal":"has_value","percent":"rate%%value","slash":"path\\value","apxy/act/-/id":"%s"}', 'exact', CURRENT_TIMESTAMP, '2024-01-01T00:00:00Z')`, exactID, exactID),
		fmt.Sprintf(`INSERT INTO actors (id, name, namespace, labels, external_id, created_at, updated_at) VALUES ('%s', 'acme-prod', 'root', '{}', 'prefix', CURRENT_TIMESTAMP, '2025-01-01T00:00:00Z')`, prefixID),
		fmt.Sprintf(`INSERT INTO actors (id, name, namespace, labels, external_id, created_at, updated_at) VALUES ('%s', 'billing', 'root', '{"team":"acme"}', 'label', CURRENT_TIMESTAMP, '2026-01-01T00:00:00Z')`, labelID),
		...
		fmt.Sprintf(`INSERT INTO actors (id, name, namespace, labels, external_id, created_at, updated_at, deleted_at) VALUES ('%s', 'acme-deleted', 'root', '{}', 'deleted', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)`, deletedID),
	}
	...
	require.Equal(t, labelID.String(), all.Items[2].ResourceID)
	require.Equal(t, 2, all.Items[2].MatchRank)
	require.Equal(t, []SearchLabelMatch{{Key: "team", Value: "acme"}}, all.Items[2].MatchedLabels)

	for _, literalQuery := range []string{"%", `\`} {
...
func TestSearchResourcesDistinguishesDuplicateNamesAcrossNamespaces(t *testing.T) {
	_, db, raw := MustApplyBlankTestDbConfigRaw(t, nil)

	firstID := apid.New(apid.PrefixActor)
	secondID := apid.New(apid.PrefixActor)
	for _, statement := range []string{
		`INSERT INTO namespaces (path, depth, state, created_at, updated_at) VALUES ('root.one', 1, 'active', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)`,
		`INSERT INTO namespaces (path, depth, state, created_at, updated_at) VALUES ('root.two', 1, 'active', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)`,
		fmt.Sprintf(`INSERT INTO actors (id, name, namespace, external_id, created_at, updated_at) VALUES ('%s', 'shared', 'root.one', 'first', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)`, firstID),
		fmt.Sprintf(`INSERT INTO actors (id, name, namespace, external_id, created_at, updated_at) VALUES ('%s', 'shared', 'root.two', 'second', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)`, secondID),
	} {
		_, err := raw.Exec(statement)
		require.NoError(t, err)
	}
	...
	require.NoError(t, err)
	require.Len(t, result.Items, 2)
	require.ElementsMatch(t, []string{"root.one", "root.two"}, []string{result.Items[0].Namespace, result.Items[1].Namespace})
	require.NotEqual(t, result.Items[0].ResourceID, result.Items[1].ResourceID)
}
...
func TestSearchResourcesCombinesTextSelectorAndNamespace(t *testing.T) {
	...
	require.Equal(t, "payments-api", result.Items[0].Name)
...
func TestSearchResourcesCollapsesConnectorDefinitionVersions(t *testing.T) {
	...
	require.Equal(t, "connector-match", result.Items[0].Name)
}

func TestSearchResourcesEmptyNamespaceMatchersNeverMeansUnrestricted(t *testing.T) {
	_, db, raw := MustApplyBlankTestDbConfigRaw(t, nil)
	id := apid.New(apid.PrefixActor)
	_, err := raw.Exec(fmt.Sprintf(`INSERT INTO actors (id, name, namespace, labels, external_id, created_at, updated_at) VALUES ('%s', 'visible', 'root', '{}', 'actor', CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)`, id))
	require.NoError(t, err)
...
func TestSearchResourcesSeedCoversNamespaceKeyAndRateLimit(t *testing.T) {
	...
	for _, statement := range []string{
		`INSERT INTO namespaces (path, depth, state, labels, created_at, updated_at) VALUES ('root.seed', 1, 'active', '{"apxy/ns/-/ns":"root.seed"}', CURRENT_TIMESTAMP, '2024-01-01T00:00:00Z')`,
		fmt.Sprintf(`INSERT INTO keys (id, name, namespace, usage, material_type, state, labels, created_at, updated_at) VALUES ('%s', 'seed-key', 'root.seed', 'data_encryption', 'symmetric', 'active', '{"apxy/key/-/id":"%s"}', CURRENT_TIMESTAMP, '2025-01-01T00:00:00Z')`, keyID, keyID),
		fmt.Sprintf(`INSERT INTO rate_limits (id, name, namespace, definition, labels, created_at, updated_at) VALUES ('%s', 'seed-limit', 'root.seed', '{}', '{"apxy/rl/-/id":"%s"}', CURRENT_TIMESTAMP, '2026-01-01T00:00:00Z')`, rateLimitID, rateLimitID),
	} {
		_, err := raw.Exec(statement)
		require.NoError(t, err)
	}
	...
	tests := []struct {
		...
		{resourceType: SearchResourceTypeNamespace, resourceID: namespacePath, name: "seed"},
		{resourceType: SearchResourceTypeKey, resourceID: keyID.String(), name: "seed-key"},
		{resourceType: SearchResourceTypeRateLimit, resourceID: rateLimitID.String(), name: "seed-limit"},
	}
