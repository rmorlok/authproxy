...
func (co ClientOptions) ApiNameForErr() string {
...
func NewClient(options ClientOptions) (*Client, error) {
	...
	if err != nil ||
		...
		return nil, fmt.Errorf(
			"a valid %s service URL without credentials, query or fragment is required",
			options.ApiNameForErr(),
		)
	}
...
// pathToOptionalGeneration returns a path to a resource with an optional
// generation. If no generation is specified, it returns the path to just
// the id.
func pathToOptionalGeneration(collection, id string, generation uint64) string {
	path := collection + "/" + url.PathEscape(id)
	if generation != 0 {
		path += "/generations/" + strconv.FormatUint(generation, 10)
	}
...
func (c *Client) Resolve(
	...
	if id != "" {
		live, err = c.get(
			ctx,
			doc.Kind,
			pathToOptionalGeneration(adapter.Collection, id, 0),
		)
		var apiErr *APIError
		if errors.As(err, &apiErr) &&
			...
			doc.Metadata.ID == "" &&
			doc.Kind == "Namespace" {
			// Namespaces are the only resource type that you can define the id
			// directly, so a lookup by id for a namespace could mean it just
	...
	if (m.ID != "" && m.ID != live.Metadata.ID) ||
		(m.Name != "" && m.Name != live.Metadata.Name) ||
		(m.Namespace != "" && m.Namespace != live.Metadata.Namespace) {
		// This is a sanity check to make sure we didn't pull the wrong resource
	...
	if m.Generation != 0 {
		...
		if live.Metadata.ID != resolvedID ||
			live.Metadata.Generation != m.Generation ||
			(m.ID != "" && m.ID != live.Metadata.ID) ||
			(m.Name != "" && m.Name != live.Metadata.Name) ||
			(m.Namespace != "" && m.Namespace != live.Metadata.Namespace) {
...
func (c *Client) findByName(
	...
	for {
		...
		if page.APIVersion != meta.APIVersionV1Alpha1 ||
			page.Kind != apiv1alpha1.ListKind(kind) ||
			page.Items == nil {
		...
		for _, item := range page.Items {
			...
			if live.Metadata.Namespace != m.Namespace ||
				live.Metadata.Name != m.Name {
				continue
			}
...
func (c *Client) request(
	...
	query url.Values,
	body []byte,
) (data []byte, redacted bool, err error) {
...
func (c *Client) createBody(doc Document) ([]byte, error) {
	...
	if doc.Kind == "Connection" {
		// Connection is not currently supported for create
...
func (c *Client) Update(
	...
	data, redacted, err := c.request(
		...
		pathToOptionalGeneration(
			adapter.Collection,
			target.Current.Metadata.ID,
			target.Document.Metadata.Generation,
		),
		nil, // query
		patch,
	)
