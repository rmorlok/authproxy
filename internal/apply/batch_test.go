...
func newBatchServer(t *testing.T) (*batchServer, *Client) {
	s := &batchServer{t: t, objects: map[string]map[string]any{}}
	s.objects["namespaces/root"] = map[string]any{"apiVersion": string(meta.APIVersionV1Alpha1), "kind": "Namespace", "metadata": map[string]any{"id": "root", "name": "root"}, "spec": map[string]any{}}
...
func (s *batchServer) handle(w http.ResponseWriter, r *http.Request) {
	...
	kinds := map[string]string{"namespaces": "Namespace", "actors": "Actor", "keys": "Key", "connectors": "Connector", "rate-limits": "RateLimit", "connections": "Connection"}
	switch r.Method {
	case "GET":
...
func asSpec(object map[string]any) map[string]any { m, _ := object["spec"].(map[string]any); return m }
...
func TestBatchMissingPrerequisitesAndCyclesPreventAllWrites(t *testing.T) {
	for _, tc := range []struct {
		...
		{"missing namespace", func(t *testing.T) []Document {
			return []Document{batchDoc(t, "Actor", "valid", "root", `{"externalId":"subject"}`), batchDoc(t, "Actor", "missing", "root.absent", `{"externalId":"subject"}`)}
		}, "prerequisite"},
		{"missing explicit reference", func(t *testing.T) []Document {
			return []Document{batchDoc(t, "Namespace", "team", "root", `{"encryptionKeyRef":{"apiVersion":"authproxy.net/v1alpha1","kind":"Key","name":"missing","namespace":"root"}}`)}
		}, "not found"},
		{"namespace key cycle", func(t *testing.T) []Document {
			return []Document{batchDoc(t, "Namespace", "team", "root", `{"encryptionKeyRef":{"apiVersion":"authproxy.net/v1alpha1","kind":"Key","name":"key","namespace":"root.team"}}`), batchDoc(t, "Key", "key", "root.team", `{"keyData":{"value":"secret"}}`)}
		}, "cycle"},
	} {
...
func TestBatchCancellationStopsMutations(t *testing.T) {
	server, client := newBatchServer(t)
	docs := []Document{batchDoc(t, "Actor", "first", "root", `{"externalId":"first"}`), batchDoc(t, "Actor", "second", "root", `{"externalId":"second"}`)}
...
func TestBatchConnectionCreationAndLateInvalidPlanPreventWrites(t *testing.T) {
	for _, last := range []Document{
		...
		server, client := newBatchServer(t)
		_, err := client.Prepare(context.Background(), []Document{batchDoc(t, "Actor", "valid", "root", `{"externalId":"subject"}`), last}, ReconcileOptions{Overwrite: true})
		require.Error(t, err)
		require.Empty(t, server.writes)
	}
}
...
func TestBatchPrevalidatesOverwriteConflicts(t *testing.T) {
	...
	live := withHistory(t, reconcileLive(t, "Actor", apid.New(apid.PrefixActor).String(), `{"externalId":"subject"}`, map[string]string{"managed": "changed"}), clientDoc(t, "Actor", "  name: example\n  namespace: root\n  annotations: {managed: original}", `{"externalId":"subject"}`))
...
func TestBatchExternalReferenceAndConflictingAlias(t *testing.T) {
	for _, mismatched := range []bool{false, true} {
		...
		docs := []Document{batchDoc(t, "Namespace", "team", "root", ref)}
		if mismatched {
