...
func (c *Client) resolveApplyTarget(
	ctx context.Context,
	doc Document,
) (*LiveResource, error) {
...
func (c *Client) selectGenerationTarget(
	ctx context.Context,
	doc Document,
	selected *LiveResource,
	...
	plan, planErr := Reconcile(
		Target{Document: doc, Current: selected},
	...
	selection, err := descriptor.Generations.Select(
		doc.Resource,
		selected.Resource,
		changed,
		editable != nil,
...
func (c *Client) resourceGenerations(
	ctx context.Context,
	selected *LiveResource,
	descriptor registry.ResourceType,
) (editable, newest *LiveResource, err error) {
	...
	for {
		...
		for _, item := range page.Items {
			...
			case meta.GenerationEditable:
				...
				editable = live
			case meta.GenerationPublished,
				meta.GenerationHistorical:
			default:
				return nil, nil, fmt.Errorf("invalid generation lifecycle classification")
			}

			if newest == nil ||
				live.Metadata.Generation > newest.Metadata.Generation {
				newest = live
			}
		}
