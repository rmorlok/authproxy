...
func (o PruneOptions) kinds() ([]meta.Kind, error) {
	...
	for _, value := range o.Allowlist {
		...
		if kind != "Actor" && kind != "Key" && kind != "RateLimit" {
...
func (c *Client) listAll(ctx context.Context, kind meta.Kind, namespace string) ([]*LiveResource, error) {
	...
	for {
		data, redacted, err := c.request(ctx, http.MethodGet, descriptor.Collection, query, nil)
		if err != nil {
			return nil, err
		}
		var page apiv1.ResourceList[json.RawMessage]
		if util.DecodeJSONStrict(data, &page) != nil || page.APIVersion != meta.APIVersionV1Alpha1 || page.Kind != apiv1.ListKind(kind) || page.Items == nil {
...
func (b *Batch) PreparePrune(ctx context.Context, options PruneOptions) (*PrunePlan, error) {
	...
	p := &PrunePlan{client: b.client, options: options, batch: b}
	for _, kind := range kinds {
		...
		for _, resource := range live {
			if resource.Metadata.Namespace != options.Namespace || !selector.Matches(resource.Metadata.Labels) || keep[string(kind)+"/"+resource.Metadata.ID] {
				continue
			}
...
func (p *PrunePlan) checkReferences(ctx context.Context) error {
	...
	for _, resource := range resources {
		...
		for _, ref := range refs {
			for _, candidate := range p.candidates {
				if ref.Kind == candidate.Kind && (ref.ID == candidate.Metadata.ID || ref.HasNamespacedName() && ref.Namespace == candidate.Metadata.Namespace && ref.Name == candidate.Metadata.Name) {
...
func (p *PrunePlan) Execute(ctx context.Context) ([]Result, error) {
	if p.batch == nil || !p.batch.completedSuccessfully.Load() {
	...
	for _, resource := range p.candidates {
		...
		path := pathToOptionalGeneration(descriptor.Collection, resource.Metadata.ID, 0)
		result := Result{Source: "prune", Kind: resource.Kind, Identity: resource.Metadata.ID, Operation: Operation("delete"), Status: "pruned"}
		deletePath := path
