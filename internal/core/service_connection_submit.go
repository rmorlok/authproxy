...
func (c *connection) SubmitForm(ctx context.Context, req iface.SubmitConnectionRequest) (iface.InitiateConnectionResponse, error) {
	...
	if err != nil {
		return nil, httperr.InternalServerError(httperr.WithInternalErr(fmt.Errorf("failed to get current step: %w", err)))
	}

	// Get existing configuration to merge into
	existingConfig, err := c.GetConfiguration(ctx)
	if err != nil {
		return nil, httperr.InternalServerError(httperr.WithInternalErr(fmt.Errorf("failed to get existing configuration: %w", err)))
	}
	...
	if err := c.SetConfiguration(ctx, mergedConfig); err != nil {
		return nil, httperr.InternalServerError(httperr.WithInternalErr(fmt.Errorf("failed to save configuration: %w", err)))
	}

	// Determine the next step
	nextStep, err := connector.SetupFlow.NextSetupStep(*setupStep)
	if err != nil {
		return nil, httperr.InternalServerError(httperr.WithInternalErr(fmt.Errorf("failed to determine next step: %w", err)))
	}
	...
	if nextStep == "" {
		...
		if err := c.SetSetupStep(ctx, nil); err != nil {
			return nil, httperr.InternalServerError(httperr.WithInternalErr(fmt.Errorf("failed to clear setup step: %w", err)))
		}

		if err := c.SetState(ctx, database.ConnectionStateReady); err != nil {
			return nil, httperr.InternalServerError(httperr.WithInternalErr(fmt.Errorf("failed to set connection state to ready: %w", err)))
		}
...
func (c *connection) initiateAuthStep(ctx context.Context, returnToUrl string, connector *cschema.Connector) (iface.InitiateConnectionResponse, error) {
	...
	if err := c.SetSetupStep(ctx, &authStep); err != nil {
		return nil, httperr.InternalServerError(httperr.WithInternalErr(fmt.Errorf("failed to set setup step to auth: %w", err)))
	}
	...
	if err != nil {
		return nil, httperr.InternalServerError(httperr.WithInternalErr(fmt.Errorf("failed to generate OAuth redirect URL: %w", err)))
	}
...
func (c *connection) buildFormResponse(ctx context.Context, setupStep string, sf *cschema.SetupFlow) (iface.InitiateConnectionResponse, error) {
	...
	if err != nil {
		return nil, httperr.InternalServerError(httperr.WithInternalErr(fmt.Errorf("failed to get step definition: %w", err)))
	}

	if err := c.SetSetupStep(ctx, &setupStep); err != nil {
		return nil, httperr.InternalServerError(httperr.WithInternalErr(fmt.Errorf("failed to update setup step: %w", err)))
	}
