...
func (s *service) applyMigrateConnectionGenerationV1(
	ctx context.Context,
	connectionID apid.ID,
	targetGeneration uint64,
) error {
	logger := s.logger.With(
		"workflow", WorkflowNameMigrateConnectionGenerationV1,
		"activity", ActivityNameMigrateConnectionGenerationApplyV1,
		"connection_id", connectionID,
		"target_generation", targetGeneration,
	)
	logger.Info("connection generation migration apply activity started")
	defer logger.Info("connection generation migration apply activity completed")

	// Compute the set of changes that are needed to move the connection from
	// the current generation to the taget generation. This call is the primary
	...
	updated, err := s.db.UpdateConnectionForGenerationMigration(
		ctx,
		database.ConnectionGenerationMigrationUpdate{
			Id:                     connectionID,
			ConnectorId:            candidate.Connection.ConnectorId,
			ConnectorGeneration:    targetGeneration,
	...
	logger.Info(
		"connection generation migration applied",
		"source_generation", candidate.Connection.ConnectorGeneration,
		"target_generation", updated.ConnectorGeneration,
	)
	return nil
}
...
// after a connection generation migration. Not all connection types implement
