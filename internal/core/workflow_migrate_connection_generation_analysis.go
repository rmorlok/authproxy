...
// from the source connection to the migrated generation, and identifies gaps. If
...
func applySetupFieldMigrationAnalysis(
	...
	fields []cschema.SetupField, // the fields we need on the target generation
	sourceFields map[string]bool, // the set of fields that are present on the source generation
) error {
...
// still contain a stale reauthentication notice from the source generation.
...
func migrationNotificationMetadata(
	...
	return map[string]any{
		...
		"target_generation": candidate.Target.Generation,
