...
func GenerationPolicy() meta.GenerationPolicy[Connector, ConnectorPatch] {
	return meta.GenerationPolicy[Connector, ConnectorPatch]{
		...
		ChangesGeneration: func(p *ConnectorPatch) bool { return p.Spec != nil && (p.Spec.HasDefinition() || p.Spec.HasRelease()) },
...
func selectConnectorGeneration(desired, selected *Connector, changed, hasEditable bool) (meta.GenerationSelection, error) {
	if !changed && (!hasEditable || desired.Spec.Release.DesiredState == ConnectorReleaseStatePrimary) {
		return meta.GenerationSelection{Source: meta.GenerationSelected}, nil
	}
	ctx := connectorGenerationContext{
		publishDefinition: desired.Spec.Release.DesiredState == ConnectorReleaseStatePrimary && selected.Status != nil && selected.Status.Release.State == ConnectorReleaseStatePrimary,
	}
	if hasEditable {
...
func finalizeConnectorGenerationPatch(desired, current *Connector, patch *ConnectorPatch, selectionContext any, explicit bool) (*ConnectorPatch, error) {
	...
	setRelease := func(state ConnectorReleaseState) { spec.Release = &ConnectorReleaseSpecPatch{DesiredState: &state} }
	...
	if !explicit && state == ConnectorReleaseStatePrimary && current.Status != nil && current.Status.Release.State != ConnectorReleaseStatePrimary {
		setRelease(state)
	}
	...
	if explicit && (*result.Metadata != (meta.ObjectMetaPatch{}) || spec.HasDefinition() || spec.HasRelease()) {
		if current.Status == nil || current.Status.Release.State != ConnectorReleaseStateDraft {
