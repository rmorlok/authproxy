			...
			Schema: `
{
  ...
  "properties": {
	"test": {
		"$ref": "./schema.json#/$defs/IntegerValue"
    }
  }
}`,
			Tests: []test{
				...
				},
				{
					Name:  "value - other attributes",
					Valid: false,
					Data:  `{"test": {"value": 99, "other": "value"}}`,
				},
				{
					Name:  "env var",
					Valid: true,
  ...
  "properties": {
	...
	for _, entity := range entities {
		t.Run(entity.Name, func(t *testing.T) {
			...
			require.Equal(t, sid, SchemaIdCommon, "schema ID should be the same as the one in the schema")

			err := c.AddResource(
				"https://raw.githubusercontent.com/rmorlok/authproxy/refs/heads/main/schema/common/test.json",
				strings.NewReader(strings.TrimSpace(entity.Schema)),
			)
			require.NoError(t, err)

			// Compile the test schema
			schema, err := c.Compile("https://raw.githubusercontent.com/rmorlok/authproxy/refs/heads/main/schema/common/test.json")
			require.NoError(t, err)
