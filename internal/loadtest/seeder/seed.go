...
const (
	defaultProviderBaseURL = "http://go-oauth2-server"
...
func Seed(ctx context.Context, opts Options) (*Result, error) {
	...
	connectorDef := connectorDefinition(opts.Profile.Name, providerBaseURL)
	connectorJSON, err := smeta.CanonicalSpecJSON(connectorDef)
	if err != nil {
	...
	if opts.ProviderClientBootstrap {
		...
		if err := ensureProviderLoadtestClient(ctx, opts.ProviderHTTPClient, providerBaseURL); err != nil {
			return nil, err
		}
	}
	if existingConnector == nil {
...
func connectorDefinition(profileName, providerBaseURL string) *cschema.ConnectorDefinition {
	...
	def := &cschema.ConnectorDefinition{
		...
		Auth: &cschema.Auth{InnerVal: &cschema.AuthOAuth2{
			...
			ClientId:                common.NewStringValueDirectInline(loadtestProviderClientID),
			ClientSecret:            common.NewStringValueDirectInline(loadtestProviderClientSecret),
			...
			Token: cschema.AuthOauth2Token{
				Endpoint:                providerBaseURL + "/v1/oauth/tokens",
