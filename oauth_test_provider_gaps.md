...
| P2 | G11 | Account identity simulation | Provider can return stable or changed userinfo/profile responses across reconnects, and AuthProxy can persist/compare that identity when a connector requires stable provider identity. | #182 — identity changes | Add `/v1/oauth/userinfo` endpoint or extend introspection with `sub` and `email`. Allow the test mode to switch browser sessions between provider users. Add AuthProxy connector schema, persistence, and re-auth mismatch handling described in [`oauth_provider_identity_changes.md`](oauth_provider_identity_changes.md). |
