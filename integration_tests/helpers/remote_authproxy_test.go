...
func TestRemoteAuthProxySignsWithGlobalKeyAndProvisionsUser(t *testing.T) {
	...
	scopedClaims := captureRemoteAuthProxyClaims(t, rig, globalKey, "scoped-admin", "root.smoke-admin")
	assert.Equal(t, "root.smoke-admin", scopedClaims.Namespace)
	assert.Equal(t, AllPermissionsForNamespace("root.smoke-admin"), scopedClaims.Permissions)
}
