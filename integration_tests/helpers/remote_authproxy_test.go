...
func TestRemoteAuthProxySignsWithGlobalKeyAndProvisionsActors(t *testing.T) {
	...
	assert.Equal(t, rig.UserActorExternalID, userClaims.Actor.Spec.ExternalId)
	assert.Equal(t, rig.UserActorNamespace, userClaims.Actor.Metadata.Namespace)
	assert.Equal(t, userPermissions, userClaims.Actor.Spec.Permissions)
	...
	assert.Equal(t, rig.AdminActorExternalID, adminClaims.Actor.Spec.ExternalId)
	assert.Equal(t, rig.AdminActorNamespace, adminClaims.Actor.Metadata.Namespace)
	assert.Equal(t, adminPermissions, adminClaims.Actor.Spec.Permissions)
