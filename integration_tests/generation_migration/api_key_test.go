...
const (
	...
	apiKeyMigrationCredentialKey = "generation-migration-api-key"
)

func TestApiKeyGenerationMigrationConfigureChangeRequiresSetup(t *testing.T) {
	...
	published := env.PublishConnectorGeneration(t, connectorID, v2, nil, nil)
	require.Equal(t, uint64(2), published.Metadata.Generation)

	env.MigrateConnectionGenerationAndWait(t, connectionID, 2, apiKeyMigrationTimeout)
	...
	require.Equal(t, uint64(2), afterSetup.ConnectorGeneration)
...
func TestApiKeyGenerationMigrationPreconnectChangeRequiresReauth(t *testing.T) {
	const rotatedKey = "generation-migration-api-key-reauth"
