...
func TestNoAuthGenerationMigrationAppliesDefaultsAndRunsAllTargetProbes(t *testing.T) {
	...
	require.Equal(t, uint64(1), initial.ConnectorGeneration)
	...
	published := env.PublishConnectorGeneration(
		t,
		created.GetId(),
		noAuthMigrationConnectorDefinition(
	...
	migration := env.MigrateConnectionGenerationAndWait(t, connectionID, 2, noAuthMigrationTimeout)
