...
func TestOAuth2GenerationMigrationScopeExpansionRequiresReauth(t *testing.T) {
	...
	rig.publishRequiredScopeGeneration(t, []string{"read", "write"})
	rig.provider.Script(rig.clientKey, helpers.EndpointRefresh, helpers.ScriptAction{
		ScopeOverride: stringPointer("read"),
	})

	rig.env.MigrateConnectionGenerationAndWait(t, connectionID, 2, oauthMigrationTimeout)

	migrated := rig.env.GetConnection(t, connectionID)
	require.Equal(t, uint64(2), migrated.ConnectorGeneration)
	...
	assert.NotEqual(t, initialToken.Id, migrationToken.Id,
		"the target-generation refresh should persist the provider's granted scope set")
	...
	require.Equal(t, uint64(2), afterReauth.ConnectorGeneration)
...
func TestOAuth2GenerationMigrationScopeExpansionRollbackRestoresConnection(t *testing.T) {
	...
	require.Equal(t, uint64(2), requiresReauth.ConnectorGeneration)
	...
	rollback := rig.env.MigrateConnectionGenerationAndWait(t, connectionID, 1, oauthMigrationTimeout)
	...
	require.Equal(t, uint64(1), restored.ConnectorGeneration)
...
func newOAuthMigrationRig(t *testing.T, name string) *oauthMigrationRig {
	...
	primary := env.ForceConnectorGenerationState(t, created.GetId(), created.Metadata.Generation, connectors.ConnectorReleaseStatePrimary)
...
func (r *oauthMigrationRig) createHealthyReadConnection(t *testing.T) string {
	...
	require.Equal(t, uint64(1), conn.ConnectorGeneration)
...
func (r *oauthMigrationRig) publishRequiredScopeGeneration(t *testing.T, scopes []string) {
	t.Helper()

	published := r.env.PublishConnectorGeneration(
		t,
		r.connectorID,
