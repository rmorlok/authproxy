...
- Promote generation 1 to primary.
...
- Publish generation 2 with one new required configure field, `workspace`.

## Flow

1. Start the core workflow worker in-process.
2. Call `POST /connections/{id}/_migrateGeneration` targeting generation 2.
3. Wait for the durable task to complete.
4. Submit the `configure-workspace` form with a workspace value.

## Assertions

- Migration switches the connection to connector generation 2.
