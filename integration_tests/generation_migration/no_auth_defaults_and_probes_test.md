...
Create a no-auth connector generation 1 with an `existing` HTTP probe and
...
Publish connector generation 2 with the same `existing` probe, an additional
`added` HTTP probe, and a configure field named `region` that is required but
has the JSON Schema default `us-east-1`.

## Flow

1. Migrate the configured connection from generation 1 to generation 2 through
   `POST /connections/{id}/_migrateGeneration` and wait for the durable workflow
   task to complete.
2. Record requests to the retained and newly added probe endpoints.

## Assertions

- The connection is on generation 2, configured, healthy, and has no pending
