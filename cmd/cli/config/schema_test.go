	...
	tests := []struct {
	...
	}{
		{
			...
			Cases: []schemaTest{
				...
				{Name: "string admin_username", Valid: true, Data: `{"adminUsername": "bob"}`},
				{Name: "object admin_username via env var", Valid: true, Data: `{"adminUsername": {"envVar": "USER"}}`},
				{Name: "object admin_username via env var with default", Valid: true, Data: `{"adminUsername": {"envVar": "USER", "default": "bob"}}`},
				{Name: "object admin_username via path", Valid: true, Data: `{"adminUsername": {"path": "/etc/user"}}`},
				{Name: "object admin_username via template_env_vars", Valid: true, Data: `{"adminUsername": {"templateEnvVars": "{{ENV}}-{{USER}}"}}`},
				{Name: "extra root property", Valid: false, Data: `{"unknown": "value"}`},
				{Name: "non-object root", Valid: false, Data: `"not an object"`},
				{Name: "admin_username bad form", Valid: false, Data: `{"adminUsername": {"unknownForm": "x"}}`},
				{Name: "server present and well-formed", Valid: true, Data: `{"server": {"api": "http://localhost:8081"}}`},
				{Name: "server with all fields", Valid: true, Data: `{"server": {"api": "http://localhost:8081", "adminApi": "http://localhost:8082", "auth": "http://localhost:8080", "marketplace": "http://localhost:5173", "adminUi": "http://localhost:5174"}}`},
				...
				{Name: "signing_proxy with port string", Valid: true, Data: `{"signingProxy": {"port": "8898"}}`},
				{Name: "signing_proxy with port via env var", Valid: true, Data: `{"signingProxy": {"port": {"envVar": "SP_PORT", "default": "8888"}}}`},
				{Name: "signing_proxy extra property rejected", Valid: false, Data: `{"signingProxy": {"port": "8898", "bogus": "x"}}`},
				{Name: "admin_private_key_path as env var", Valid: true, Data: `{"adminPrivateKeyPath": {"envVar": "ADMIN_KEY_PATH"}}`},
				{Name: "admin_shared_key_path as path", Valid: true, Data: `{"adminSharedKeyPath": {"path": "~/.authproxy/shared.key"}}`},
				{Name: "admin_username inline bool coerced", Valid: true, Data: `{"adminUsername": true}`},
				{Name: "admin_username inline number coerced", Valid: true, Data: `{"adminUsername": 99}`},
				{Name: "admin_username object with both value and env_var rejected", Valid: false, Data: `{"adminUsername": {"value": "x", "envVar": "Y"}}`},
				{Name: "admin_username object with value and extra rejected", Valid: false, Data: `{"adminUsername": {"value": "x", "extra": "y"}}`},
			},
		},
		{
			Def: "Server",
			Cases: []schemaTest{
				...
				{Name: "all urls", Valid: true, Data: `{"api": "http://localhost:8081", "adminApi": "http://localhost:8082", "auth": "http://localhost:8080", "marketplace": "http://localhost:5173", "adminUi": "http://localhost:5174"}`},
				{Name: "api as env var", Valid: true, Data: `{"api": {"envVar": "API_URL", "default": "http://localhost:8081"}}`},
		...
		},
		{
			...
			Cases: []schemaTest{
				...
				{Name: "port via env var with default", Valid: true, Data: `{"port": {"envVar": "AUTHPROXY_SIGNING_PROXY_PORT", "default": "8888"}}`},
