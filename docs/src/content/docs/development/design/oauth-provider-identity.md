---
title: OAuth provider identity changes
description: Proposed stable-provider-identity behavior for OAuth reauthentication; this behavior is not yet implemented.
banner:
  content: Proposal only — AuthProxy does not currently enforce stable provider identity during OAuth reauthentication.
---
