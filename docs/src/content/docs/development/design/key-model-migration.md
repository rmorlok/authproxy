---
title: Unified key model migration (historical)
description: Historical design and rollout record for AuthProxy's unified key and data-encryption-key model.
pagefind: false
banner:
  content: This is a historical migration record, not current deployment guidance.
---

This document preserves the design and rollout contract used for the key model
migration tracked by [#605](https://github.com/rmorlok/authproxy/issues/605).
The unified `keys` and `data_encryption_keys` model described here is now in the
main codebase; branch names and sequencing below are retained only to explain
how the migration was delivered.

## Historical branching plan
...
See [integration_tests/README.md](https://github.com/rmorlok/authproxy/blob/main/integration_tests/README.md) for provider
