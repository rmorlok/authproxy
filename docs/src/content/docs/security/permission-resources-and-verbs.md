...
| `keys` | `create`, `delete`, `get`, `list`, `update` | Reusable signing and encryption-key resources; `delete` also gates `DELETE /keys/{id}/unused`, which rejects namespace references and encryption history |
