---
title: Connector Generation Migrations
description: Move live connections between connector generations with migration hooks, automatic analysis, rollback, and notifications.
---

Connector generation migrations move an existing connection from one published
generation of a connector to another generation of the same connector. The migration
...
logical connector, not an individual definition generation. Every source and
target generation therefore projects the same connector name. A generation migration
changes the connection's recorded generation but does not rename the connector or
...
A connector generation can define shared JavaScript and optional migration hooks:
...
until a connection migration workflow crosses that generation.
...
Rollback is the same workflow pointed at an earlier connector generation.
    ...
    set: [
      {
        ...
        message: "Review this connection before using the new connector generation.",
...
- If the migration is not blocked by required setup or re-authentication,
  AuthProxy runs the target connector's enabled probes once after the generation
  switch.

The workflow writes the connector generation, encrypted configuration, labels,
...
```http
POST /api/v1/connections/{id}/_migrateGeneration
