...
const params: ApiSessionInitiateRequest = {
    // Always use the cleaned URL (without authToken) as the returnToUrl
    returnToUrl: (authTokenFromQuery ? (() => { const u = new URL(window.location.href); u.searchParams.delete('authToken'); return u.toString(); })() : window.location.href),
};
...
if ((window as any).AUTHPROXY_AUTH_TOKEN) {
    ...
    params.authToken = authTokenFromQuery;
}
