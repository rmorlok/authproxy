...
describe('ConnectorVersionDetail', () => {
  ...
  it('shows namespace and edits draft metadata through the shared menu', async () => {
    ...
    vi.mocked(connectors.updateGeneration).mockResolvedValue({
      status: 200,
      data: {...connectorVersion, metadata: {...connectorVersion.metadata, labels: {...connectorVersion.metadata.labels, tier: 'internal'}}},
    } as any);
    ...
    expect(within(namespace!).getByText(connectorVersion.metadata.namespace)).toBeTruthy();
    ...
    await waitFor(() => expect(connectors.updateGeneration).toHaveBeenCalledWith(
      connectorVersion.metadata.id,
      connectorVersion.metadata.generation,
      {
  ...
  it('keeps metadata actions read-only outside the draft version', async () => {
    const user = userEvent.setup();
    render(<ConnectorVersionDetail connectorVersion={{...connectorVersion, status: {release: {state: ConnectorReleaseState.PRIMARY}}}}/>);
